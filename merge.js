...
const smtPath = path.join(__dirname, 'frontend/src/pages/admin/SMTPlayer.jsx');
const bookIndexPath = path.join(__dirname, 'frontend/src/pages/admin/BookIndex.jsx');

const smtContent = fs.readFileSync(smtPath, 'utf8');
let bookIndexContent = fs.readFileSync(bookIndexPath, 'utf8');

// 1. Extract Helper Components & Constants 
const helperStart = smtContent.indexOf('const DividerBox');
const helperEnd = smtContent.indexOf('const PDFPageRender =');
let helpers = smtContent.substring(helperStart, helperEnd);

const explodeStart = helpers.indexOf('const explodeBookString');
const explodeEnd = helpers.indexOf('const DraggableWrapper');
if (explodeStart !== -1 && explodeEnd !== -1) {
    helpers = helpers.substring(0, explodeStart) + helpers.substring(explodeEnd);
}

const constantsStart = smtContent.indexOf('// Constants for PDF Highlighting Tool');
const constantsEnd = smtContent.indexOf('const ScrollMenuPopup =');
const constants = smtContent.substring(constantsStart, constantsEnd);

const insertPos1 = bookIndexContent.indexOf('const PDFPageRender =');
bookIndexContent = bookIndexContent.substring(0, insertPos1) + constants + helpers + "\n" + bookIndexContent.substring(insertPos1);

// 2. Extract Highlighting & Audio Player States
const togglePlayFixed = `
    const togglePlay = () => {
...
const insertPos2 = bookIndexContent.indexOf('const groupedBooks =');
bookIndexContent = bookIndexContent.substring(0, insertPos2) + audioAndHighlightLogic + "\n    " + bookIndexContent.substring(insertPos2);

// 3. Inject pageHighlights into PDFPageRender inside BookIndex
bookIndexContent = bookIndexContent.replace(
    'isCover={isHardCover}',
...
const hoverHeaderTarget = \`            {/* Hover Header (Menu + Back) */}
...
bookIndexContent = bookIndexContent.replace(hoverHeaderTarget, topBarCode);

// 5. Inject bottom Audio Player and Overlay Popups into rendering
const renderBottomInjections = \`
...
const bottomInsertionTarget = \`            </div>
        </div>
    );
};
...
bookIndexContent = bookIndexContent.replace(bottomInsertionTarget, renderBottomInjections);

fs.writeFileSync(bookIndexPath, bookIndexContent, 'utf8');
