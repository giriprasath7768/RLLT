...
from app.db.models import SevenTNTDayCycleChartMapping
...
async def sync_chart(
    ...
    try:
        ...
        result = await db.execute(select(SevenTNTDayCycleChartMapping).filter(
            SevenTNTDayCycleChartMapping.module == module,
            SevenTNTDayCycleChartMapping.facet == facet,
            SevenTNTDayCycleChartMapping.phase == phase
        ))
        existing = result.scalars().first()

        if logo and logo.filename:
            ext = os.path.splitext(logo.filename)[1]
            filename = f"seventnt_daycycle_logo_{module}_{facet}_{phase}_{uuid.uuid4().hex}{ext}"
        ...
        upsert_stmt = insert(SevenTNTDayCycleChartMapping).values(
            id=uuid.uuid4(),
            module=module,
            facet=facet,
            phase=phase,
...
async def list_charts(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(SevenTNTDayCycleChartMapping).order_by(SevenTNTDayCycleChartMapping.module, SevenTNTDayCycleChartMapping.facet, SevenTNTDayCycleChartMapping.phase))
...
async def delete_chart(module: int, facet: int, phase: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(delete(SevenTNTDayCycleChartMapping).where(
        SevenTNTDayCycleChartMapping.module == module,
        SevenTNTDayCycleChartMapping.facet == facet,
        SevenTNTDayCycleChartMapping.phase == phase
    ))
