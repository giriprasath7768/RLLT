...
from app.db.models import SevenTNTChartMapping
...
async def sync_chart(
    ...
    try:
        ...
        result = await db.execute(select(SevenTNTChartMapping).filter(
            SevenTNTChartMapping.module == module,
            SevenTNTChartMapping.facet == facet,
            SevenTNTChartMapping.phase == phase
        ))
        existing = result.scalars().first()

        if logo and logo.filename:
            ext = os.path.splitext(logo.filename)[1]
            filename = f"seventnt_logo_{module}_{facet}_{phase}_{uuid.uuid4().hex}{ext}"
        ...
        upsert_stmt = insert(SevenTNTChartMapping).values(
            id=uuid.uuid4(),
            module=module,
            facet=facet,
            phase=phase,
...
async def list_charts(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(SevenTNTChartMapping).order_by(SevenTNTChartMapping.module, SevenTNTChartMapping.facet, SevenTNTChartMapping.phase))
...
async def delete_chart(module: int, facet: int, phase: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(delete(SevenTNTChartMapping).where(
        SevenTNTChartMapping.module == module,
        SevenTNTChartMapping.facet == facet,
        SevenTNTChartMapping.phase == phase
    ))
