...
async def get_assessment_options(db: AsyncSession = Depends(get_db)):
...
async def bulk_create_assessments(data: AssessmentBulkCreate, db: AsyncSession = Depends(get_db)):
    db_assessments = [Assessment(**a.model_dump()) for a in data.assessments]
    db.add_all(db_assessments)
    await db.commit()
    return {"message": f"Successfully imported {len(db_assessments)} assessments"}

@router.put("/{assessment_id}", response_model=AssessmentResponse)
async def update_assessment(assessment_id: uuid.UUID, assessment: AssessmentUpdate, db: AsyncSession = Depends(get_db)):
...
async def bulk_delete_assessments(assessment_ids: List[uuid.UUID], db: AsyncSession = Depends(get_db)):
...
async def purge_assessments(name: str, location_module: str, db: AsyncSession = Depends(get_db)):
    await db.execute(
        delete(Assessment)
