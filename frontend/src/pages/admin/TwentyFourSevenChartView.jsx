...
const TwentyFourSevenChartView = () => {
    ...
    const [headerTitle, setHeaderTitle] = useState("24x7 Chart Viewer");
            ...
            if (data.state_payload) {
                try {
                    ...
                    if (Array.isArray(parsed)) {
                        setChunks(parsed);
                        if (parsed.length === 8) {
                            setHeaderTitle("24x7 Chart Viewer - 40 Days");
                        } else {
                            setHeaderTitle("24x7 Chart Viewer - 30 Days");
                        }
                    } else {
                        setChunks(generateInitialData());
                    }
    ...
    const handleExportPdf = async () => {
        try {
            ...
            const fileName = `RLLT_24x7Chart_Mod${selectedChart?.module}_Fct${selectedChart?.facet}_Phase${selectedChart?.phase}.pdf`;
            pdf.save(fileName);
                                                    ...
                                                    <tbody key={chunk.id} className="text-black font-bold text-sm rllt-condensed">
                                                        ...
                                                        {chunk.days.map((d, dIdx) => (
                                                            <tr key={d.id} className="bg-white text-center border-b-2 border-black h-[38px]">
                                                                ...
                                                                <td className="border-2 border-black p-0 bg-white uppercase font-bold leading-tight align-middle" style={{ fontSize: getFS(12) }}>{d.m3b}</td>
                                                                <td className="border-2 border-black p-0 bg-white font-bold text-black align-middle" style={{ fontSize: getFS(11) }}>{d.m3t}</td>

                                                                <td className="border-2 border-black p-1 bg-white text-left uppercase font-bold leading-tight align-middle whitespace-nowrap overflow-hidden" style={{ fontSize: getFS(13) }}>{d.m4b}</td>
                                                                <td className="border-2 border-black p-0 bg-white font-bold text-black align-middle" style={{ fontSize: getFS(11) }}>{d.m4t}</td>
...
export default TwentyFourSevenChartView;
