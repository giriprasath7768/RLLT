...
const EveningChart = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://${window.location.hostname}:8000/api/charts/sync/${module}/${facet}/${phase}`, { withCredentials: true })
            .then(res => {
                ...
                if (data.state_payload) {
                    try {
                        ...
                        if (Array.isArray(parsed)) {
                            const { eveningChunks } = splitS3Data(parsed, booksDB, chaptersDB);
                            setChunks(eveningChunks);
                        } else {
                            setChunks(generateInitialData());
                        }
                    } catch(e) {
                ...
                toast.current?.show({ severity: 'success', summary: 'Loaded', detail: 'Evening Chart loaded.', life: 2000 });
            })
            .catch(err => {
    ...
    const handleShare = async () => {
        try {
            ...
            const file = new File([blob], 'Evening_Chart.pdf', { type: 'application/pdf' });
    ...
    const executeNativeShare = async () => {
        ...
        try {
            if (navigator.share) {
                await navigator.share({ title: 'Evening Chart', files: readyShareFile });
        ...
        <div className="p-8 w-full max-w-full overflow-x-auto bg-gray-50 min-h-screen print:bg-white print:p-0 print:overflow-visible">
            ...
            <div className="bg-white shadow-xl rounded-2xl border border-gray-100 overflow-hidden mb-6 print:border-none print:shadow-none print:overflow-visible">
                <div className="bg-gradient-to-r from-[#051220] to-[#0A1F35] p-6 flex flex-col xl:flex-row justify-between items-center text-white pb-4 gap-4 print:hidden">
                    <div>
                        <h1 className="text-2xl font-black tracking-tight mb-1 text-[#E67E22]">Evening Chart</h1>
                                    ...
                                    return (
                                        <tbody key={chunk.id} className="text-black font-bold text-sm rllt-condensed">
                                            <tr className="bg-white text-center font-bold h-[30px]">
                                                ...
                                                <th className="border-2 border-black p-0 text-[10px]">TIME</th>
                                                <th className="border-2 border-black p-1 text-left pl-2 text-[14px]">EVENING SCRIPTURE</th>
                                                <th className="border-2 border-black p-0 text-[10px]">TIME</th>
                                                ...
                                                <th rowSpan={7} className="border-2 border-black p-0 align-middle">
                                                    <div className="flex items-center justify-center transform -rotate-90 text-[10px] whitespace-nowrap font-extrabold uppercase">EVENING</div>
                                                </th>
                                            </tr>
                                            ...
                                            {chunk.days.map((d) => (
                                                <tr key={d.id} className="bg-white text-center border-b-2 border-black h-[38px]">
                                                    ...
                                                    <td className="border-2 border-black p-1 text-left uppercase align-middle text-[#E67E22] text-[13px]">{d.m3b}</td>
