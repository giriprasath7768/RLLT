...
const MorningEveningChart = () => {
    ...
    const [headerTitle, setHeaderTitle] = useState("Morning & Evening Chart");
                ...
                if (data.state_payload) {
                    try {
                        ...
                        if (Array.isArray(parsed)) {
                            ...
                            if (parsed.length === 8) {
                                setHeaderTitle("40 Days Morning & Evening Chart Viewer");
                            } else {
                                setHeaderTitle("Morning & Evening Chart - 30 Days");
                            }
                        } else {
                            setChunks(generateInitialData());
                        }
    ...
    const handleExportPdf = async () => {
        try {
            ...
            const fileName = `RLLT_MorningEvening_Chart_Mod${selectedChart?.module}_Fct${selectedChart?.facet}_Phase${selectedChart?.phase}.pdf`;
            pdf.save(fileName);
        ...
        try {
            if (navigator.share) {
                await navigator.share({
                    title: 'RLLT Morning/Evening Chart',
            ...
            <div className="bg-white shadow-xl rounded-2xl border border-gray-100 overflow-hidden mb-6 print:border-none print:shadow-none print:overflow-visible">
                <div className="bg-gradient-to-r from-[#051220] to-[#0A1F35] py-3 px-6 flex flex-col xl:flex-row justify-between items-center text-white border-b-2 border-gray-100 gap-2 print:hidden">
                    <div className="flex-shrink-0 text-center xl:text-left mb-2 xl:mb-0">
                        ...
                        <p className="text-[11px] font-medium text-gray-300 uppercase tracking-widest mb-1.5">Morning & Evening Split (Read-Only)</p>
                                            ...
                                            <tbody key={chunk.id} className="text-black font-bold text-sm rllt-condensed">
                                                ...
                                                {chunk.days.map((d, dIdx) => (
                                                    <tr key={d.id} className="bg-white text-center border-b-2 border-black h-[38px]">
                                                        ...
                                                        <td className="border-2 border-black p-0 bg-white text-center uppercase font-bold leading-tight align-middle overflow-hidden whitespace-nowrap" style={{ fontSize: getFS(13) }}>
                                                            <div className="flex w-full h-full items-center">
                                                                <div className="w-1/2 text-center text-[#2ed573] whitespace-nowrap px-1">
                                                                    {d.m3b_morning}
                                                                </div>
                                                                <div className="w-1/2 text-center text-[#0033CC] whitespace-nowrap px-1">
                                                                    {d.m3b_evening}
                                                                </div>
                                                            </div>
                                                        </td>
