...
const booksDB = [
    { id: 1, name: 'GENESIS', short_form: ' GEN' },
    { id: 2, name: 'EXODUS', short_form: 'EXO' },
    { id: 3, name: 'LEVITICUS', short_form: 'LEV' },
    { id: 4, name: 'NUMBERS', short_form: 'NUM' },
    { id: 5, name: 'DEUTERONOMY', short_form: 'DEU' }
];

const selectedBooks = ["GEN", "EXO", "LEV", "NUM", "DEU"];
const allSelectedBooks = [];
for (const code of selectedBooks) {
    const match = booksDB.find(b => isBookMatch(code, b));
    if (match) {
        allSelectedBooks.push(match);
        console.log(`Matched ${code} -> ${match.name}`);
    } else {
